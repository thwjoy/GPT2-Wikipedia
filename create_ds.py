...
train_dataset = WikiDataset(None, "train", tokenizer, 1024 - max_seq_len, tokenizer.pad_token_id)
inds_train = np.arange(train_dataset.__len__(), step=5000)
...
for toks, mask, metas in tqdm(iter(train_loader)): # TODO seqneuce length and add summary 
    ...
    query = filter(tokenizer.decode(generation_output[0][toks.shape[-1]:]))
    if query is not None and len(query) > 25:
